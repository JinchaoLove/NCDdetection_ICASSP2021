...
print('# lda')
print('pComParE')
lda = Baseline(mode='lda', pre='pca', C=1, kernel='rbf')
pcompare_scores, pcompare_tst_scores = lda.cv_test(
    p_train_compares, train_wavs.ad.values, p_test_compares, test_compare_ad, cv=cv, times=times, testtimes=times)
print('ling')
lda = Baseline(mode='lda', pre='pca', C=1, kernel='rbf')
ling_scores, ling_tst_scores = lda.cv_test(
    train_ling, ling_ad, test_ling, test_ling_ad, cv=cv, times=times, testtimes=times)
print('xvec')
lda = Baseline(mode='lda', pre='pca', C=1, kernel='rbf')
xvec_scores, xvec_tst_scores = lda.cv_test(
    train_xvecs, train_xvec_labels, test_xvecs, test_xvec_labels, cv=cv, times=times, testtimes=times)
print('TFIDF')
lda = Baseline(mode='lda', pre='tfidf', C=1, kernel='rbf')
tfidf_scores, tfidf_tst_scores = lda.cv_test(
    train_df.joined_all_par_trans.values, train_df.ad.values,
    test_df.joined_all_par_trans.values, test_ad, cv=cv, times=times, testtimes=times)
print('BERT')
lda = Baseline(mode='lda', pre='pca', C=1, kernel='rbf')
bert_scores, bert_tst_scores = lda.cv_test(
    bert_features, bert_labels, tst_bert_features, tst_bert_labels, cv=cv, times=times, testtimes=times)
print('DistilBERT')
lda = Baseline(mode='lda', pre='pca', C=1, kernel='rbf')
distilbert_scores, distilbert_tst_scores = lda.cv_test(
...
np.savez('lda_scores.npz', scores)
np.savez('lda_tst_scores.npz', tst_scores)
